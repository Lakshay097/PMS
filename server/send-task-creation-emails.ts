...
async function sendTaskCreationEmails() {
  try {
    console.log('=== SENDING Task Creation Emails ===\n');
    ...
    tasksSnapshot.forEach(doc => {
      ...
      if (emailLogs && emailLogs.length > 1) {
        for (let i = 1; i < emailLogs.length; i++) {
          ...
          const subject = log[3] || '';
          if (subject.includes(taskId) && (subject.includes('New task') || subject.includes('task created'))) {
            emailFound = true;
            break;
          }
        }
