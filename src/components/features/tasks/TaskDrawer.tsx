...
export default function TaskDrawer({
  ...
  const getPriorityStyle = (priority: string) => {
    switch (priority) {
      case 'Low': return 'bg-slate-100 text-secondary';
      ...
      default: return 'bg-slate-100 text-secondary';
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center font-sans p-2 sm:p-4 bg-slate-900/50 backdrop-blur-sm pointer-events-auto">
      ...
      <motion.div
        ...
        className="relative w-full max-w-2xl max-h-[90vh] sm:max-h-[85vh] rounded-xl sm:rounded-2xl shadow-2xl flex flex-col pointer-events-auto overflow-hidden bg-surface"
      >
        {/* Header */}
        <div className="px-4 sm:px-6 py-3 sm:py-5 text-white flex items-center justify-between border-b bg-slate-800 border-slate-700">
          <div className="flex-1 min-w-0">
            ...
            <span className="text-[9px] sm:text-[10px] text-secondary font-mono block mt-0.5">
              ID: {task.TaskID} &bull; Type: {task.TaskType}
            </span>
          </div>
          <button onClick={onClose} className="text-secondary hover:text-white p-1.5 sm:p-1 rounded-lg hover:bg-slate-800 transition-colors cursor-pointer flex-shrink-0">
          ...
          <button
            ...
            className={`flex-1 py-2.5 sm:py-3 text-center text-[10px] sm:text-xs font-bold border-b-2 transition-all cursor-pointer ${
              activeTab === 'details'
                ...
                : `border-transparent text-slate-500 hover:text-slate-800 hover-surface`
            }`}
          >
        ...
        <div className="flex-1 overflow-y-auto p-3 sm:p-6 space-y-4 sm:space-y-6">
          {activeTab === 'details' ? (
            <div className="space-y-6">
              {isEditing ? (
                <div className="space-y-4 rounded-xl p-4.5 shadow-xs bg-slate-50 border-token">
                  <div className="flex justify-between items-center pb-2.5 border-b border-token">
                    <span className="text-[10px] font-bold tracking-wider flex items-center gap-1.5 text-slate-700">
                      <Edit2 size={13} className="text-[#2563EB]" /> Modify task dimensions
                    </span>
                    <button
                      type="button"
                      onClick={() => setIsEditing(false)}
                      className="text-[10px] font-bold cursor-pointer border-none bg-transparent text-slate-500 hover:text-slate-800"
                    >
                      Cancel
                    </button>
                  </div>
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold tracking-wider block text-[#64748B]">Detailed description / instructions</label>
                    <textarea
                      value={editDescription}
                      onChange={(e) => setEditDescription(e.target.value)}
                      rows={4}
                      className="w-full text-xs rounded-lg p-2.5 focus:outline-none focus:ring-1 focus:ring-[#2563EB] bg-surface border-[#E2E8F0] text-slate-800 placeholder-slate-400"
                  ...
                  {/* Stakeholders Field */}
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold tracking-wider block mb-1 text-[#64748B]">Add / remove stakeholders</label>
                    <div className="border rounded-lg p-3 max-h-40 overflow-y-auto space-y-1.5 shadow-inner border-[#E2E8F0] bg-surface">
                      {assignableUsers.length === 0 ? (
                        ...
                        assignableUsers.filter(u => u.Active && u.Role === ROLE.STAKEHOLDER).map(user => {
                          ...
                          return (
                            <label key={user.UserID} className="flex items-center space-x-2.5 p-1.5 rounded-md cursor-pointer text-xs transition-colors hover-surface text-slate-800">
                              <input
                                type="checkbox"
                                checked={isChecked}
                                onChange={() => {
                                ...
                                className="h-4 w-4 rounded transition-colors border-[#CBD5E1] text-[#2563EB] focus:ring-[#2563EB]"
                              />
                ...
                <>
                  {canEditTask && (
                    <div className="flex justify-end">
                      <button
                        ...
                        className="flex items-center space-x-1.5 text-[10px] font-bold tracking-wider py-1.5 px-3 rounded-lg shadow-2xs transition-all cursor-pointer bg-surface hover-surface border-[#CBD5E1] text-slate-700">
                  ...
                  {/* Description */}
                  <div className="space-y-1">
                    <h4 className="text-[10px] font-bold tracking-wider font-sans text-[#64748B]">Scope description</h4>
                    <div className="text-xs leading-relaxed whitespace-pre-wrap rounded-lg p-3.5 border text-slate-700 bg-slate-50 border-[#E2E8F0]">
                      {task.Description}
                    </div>
                  </div>
                </>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="p-3 rounded-lg space-y-1 bg-slate-50 border-[#E2E8F0]">
                  <span className="text-[10px] font-bold tracking-widest block text-[#64748B]">Allocated by</span>
                  <div className="flex items-center space-x-2">
                    <User size={14} className="text-secondary" />
                    <span className="text-xs font-medium truncate text-slate-800">{task.AssignedByEmail}</span>
                  </div>
                </div>

                <div className="p-3 rounded-lg space-y-2 flex flex-col justify-between bg-slate-50 border-[#E2E8F0]">
                  <div>
                    <span className="text-[10px] font-bold tracking-widest block text-[#64748B]">Assigned recipients</span>
                    <div className="flex flex-wrap gap-1 mt-1">
                      {task.AssignedToEmail.split(',').map((email, idx) => {
                        ...
                        return (
                          <span key={idx} className="inline-flex items-center text-[10px] font-semibold px-2 py-0.5 rounded border bg-slate-100 text-slate-800 border-token">
                            {trimmed}
                          </span>
                        );
                      })}
                  ...
                  {isAdminLevel(currentUser.Role) && (
                    <span className="text-[9px] px-1.5 py-0.5 rounded font-mono uppercase block w-max bg-slate-200 text-slate-700">
                      {task.AssignedToRole}
                    </span>
                  )}
                </div>
              ...
              {currentUser.Role === ROLE.STAKEHOLDER && isCurrentUserAssignee && task.Status !== 'Closed' && subordinates.length > 0 && (
                <div className="rounded-lg p-4 space-y-3 shadow-3xs bg-[#EFF6FF] border-[#BFDBFE]">
                  <div className="flex items-center space-x-1.5 font-bold text-xs tracking-wider text-blue-900">
                    <User size={14} className="text-[#2563EB]" />
                    <span>Assign / delegate to subordinate</span>
                  </div>
                  <p className="text-[11px] leading-relaxed font-semibold text-blue-700">
                    As a Stakeholder, you are authorized to assign or delegate this task to members of your team subordinates list.
                  </p>
                  
                  <div className="space-y-3">
                    <div className="border rounded-lg p-3 max-h-48 overflow-y-auto space-y-1.5 shadow-inner border-[#CBD5E1] bg-surface">
                        ...
                        <>
                          <div className="flex items-center justify-between pb-1.5 border-b border-slate-100">
                            <button
                              ...
                              className="text-[9px] font-semibold cursor-pointer border-none bg-transparent text-blue-600 hover:text-blue-800"
                            >
                              Select all
                            </button>
                            <button
                              ...
                              className="text-[9px] font-semibold cursor-pointer border-none bg-transparent text-slate-500 hover:text-slate-700"
                            >
                              Clear all
                            </button>
                          </div>
                          {subordinates.map(sub => {
                            ...
                            return (
                              <label key={sub.UserID} className="flex items-center space-x-2.5 p-1.5 rounded-md cursor-pointer text-xs transition-colors hover-surface text-slate-800">
                                <input
                                  type="checkbox"
                                  checked={isChecked}
                                  onChange={() => {
                                ...
                                <div className="flex flex-col">
                                  ...
                                  <span className="text-[9.5px] font-mono text-slate-500">{sub.Email}</span>
                                </div>
                              </label>
                            );
                          })}
              ...
              {currentUser.Role === ROLE.STAKEHOLDER && isCurrentUserAssignee && task.Status !== 'Closed' && subordinates.length > 0 && (
                <div className="rounded-lg p-4 space-y-3 shadow-3xs bg-[#F0FDF4] border-[#86EFAC]">
                  <button
                    type="button"
                    onClick={() => setShowSubtaskDivision(!showSubtaskDivision)}
                    className="flex items-center justify-between w-full text-left"
                  >
                    <div className="flex items-center space-x-1.5 font-bold text-xs tracking-wider text-green-900">
                      <FileText size={14} className="text-[#16A34A]" />
                      <span>Divide into subtasks</span>
                    </div>
                    <span className={`text-[9px] font-semibold ${showSubtaskDivision ? 'rotate-180' : ''} transition-transform text-green-700`}>▼</span>
                  </button>
                  
                  {showSubtaskDivision && (
                    <div className="space-y-3 pt-2">
                      <p className="text-[11px] leading-relaxed font-semibold text-green-700">
                        Divide this task into smaller subtasks and assign each to a different subordinate.
                      </p>
                      
                      {subtaskDivisionRows.map((row, index) => (
                        <div key={index} className="border rounded-lg p-3 space-y-2 bg-surface border-[#CBD5E1]">
                          <div className="flex justify-between items-start">
                            <div className="flex-1 space-y-2">
                              <input
                                ...
                                className="w-full rounded-lg px-2.5 py-1.5 text-xs focus:outline-none focus:ring-1 focus:ring-[#16A34A] bg-slate-50 border-[#E2E8F0] text-slate-700 placeholder-slate-400"
                              />
                            ...
                            <button
                              ...
                              className="p-1 rounded cursor-pointer border-none text-red-500 hover:text-red-700 hover:bg-red-50"
                            >
                              <Trash size={14} />
                            </button>
                          </div>
                      ...
                      <button
                        ...
                        className="w-full border border-dashed text-[10.5px] font-bold tracking-wider px-3.5 py-2 rounded-lg cursor-pointer transition-colors bg-surface border-[#86EFAC] text-green-700 hover:bg-green-50"
                      >
                ...
                <div className="rounded-lg p-4 space-y-3 shadow-3xs bg-[#F0FDF4] border-[#86EFAC]">
                  ...
                  <div className="space-y-3">
                    {taskSubtasks.map((subtask) => {
                      ...
                      return (
                        <div key={subtask.SubtaskID} className="border rounded-lg p-3.5 space-y-2 bg-surface border-[#CBD5E1]">
                          <div className="flex items-start space-x-2.5">
                            <input
                              ...
                              className="h-4 w-4 mt-0.5 rounded transition-colors cursor-pointer border-[#CBD5E1] text-[#16A34A] focus:ring-[#16A34A]"
                            />
                            <div className="flex-1 space-y-1.5">
                              ...
                              {lastReport && (
                                <div className="border rounded px-2 py-1.5 mt-2 bg-blue-50 border-blue-100">
                                  <div className="text-[9px] font-medium mb-0.5 text-blue-700">Last update:</div>
                                  <div className="text-[10px] italic text-slate-700">"{lastReport.WorkSummary}"</div>
                                </div>
                              )}
                            </div>
                            ...
                            {onDeleteSubtask && (
                              <button
                                ...
                                className="p-1 rounded cursor-pointer border-none text-red-400 hover:text-red-600 hover:bg-red-50"
                              >
                                <Trash size={12} />
                              </button>
                            )}
              ...
              {isAdminLevel(currentUser.Role) && task.Status !== 'Closed' && (
                <div className="rounded-lg p-4 space-y-3 shadow-3xs bg-[#FFFBEB] border-[#FDE68A]">
                  <div className="flex items-center space-x-1.5 font-bold text-xs tracking-wider text-amber-950">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="space-y-2">
                      <label className="text-[9px] font-bold tracking-wider block text-[#64748B]">Reassign to user</label>
                      ...
                      <input
                        ...
                        className="w-full rounded-lg px-2.5 py-1.5 text-xs focus:outline-none focus:ring-1 focus:ring-[#D97706] bg-surface border-[#CBD5E1] text-slate-700 placeholder-slate-400"
                      />
                          ...
                          <>
                            <div className="flex items-center justify-between pb-1.5 border-b border-slate-100">
                              <button
                                ...
                                className="text-[9px] font-semibold cursor-pointer border-none bg-transparent text-amber-700 hover:text-amber-900"
                              >
                                Select all
                              </button>
                              <button
                              ...
                              return (
                                <label key={user.UserID} className="flex items-center space-x-2.5 p-1.5 rounded-md cursor-pointer text-xs transition-colors hover-surface text-slate-800">
                                  <input
                                    ...
                                    className="h-4 w-4 rounded transition-colors border-[#CBD5E1] text-[#D97706] focus:ring-[#D97706]"
                                  />
                                  <div className="flex flex-col">
                                    <span className="font-semibold text-primary">{user.FullName}</span>
                                    <span className="text-[9.5px] font-mono text-slate-500">{user.Email}</span>
                                  </div>
                                </label>
                              );
                            })}
                      ...
                      <label className="flex items-center space-x-2 text-[10px] font-semibold cursor-pointer text-slate-700">
                        <input
                          type="checkbox"
                    ...
                    {/* Team Reassignment */}
                    <div className="space-y-1">
                      <label className="text-[9px] font-bold tracking-wider block text-[#64748B]">Reassign to team</label>
                      <div className="flex gap-2">
                        <select
                          ...
                          className={`rounded-lg px-2.5 py-1.5 text-xs focus:outline-none focus:ring-1 focus:ring-[#D97706] flex-grow bg-surface border-[#CBD5E1] text-slate-700`}
                        >
              ...
              {/* Metadata details */}
              <div className="rounded-lg divide-y bg-slate-50 border-[#E2E8F0] divide-[#E2E8F0]">
                <div className="p-3 flex justify-between items-center text-xs">
                  <span className="font-medium font-sans text-slate-500">Start Date</span>
                  <span className="font-mono text-slate-800">{task.StartDate}</span>
                </div>
                <div className="p-3 flex justify-between items-center text-xs">
                  <span className="font-medium font-sans text-slate-500">Schedule Due Date</span>
                ...
                {task.CompletionDate && (
                  <div className="p-3 flex justify-between items-center text-xs bg-emerald-50/50">
                    <span className="font-medium text-emerald-700">Completed Date</span>
                    <span className="font-mono font-semibold text-emerald-800">{task.CompletionDate}</span>
                  </div>
                )}
              </div>
              ...
              {task.Status !== 'Closed' && (
                <div className="rounded-xl p-3.5 space-y-2.5 font-sans shadow-3xs bg-surface border-[#E2E8F0]">
                  <div className="flex justify-between items-center border-b pb-1.5 border-slate-100">
                    <h4 className="text-[10px] font-black tracking-wider text-[#64748B]">
                  ...
                  {((task.EtaRequestCount || 0) < 3) ? (
                    <div className="space-y-2">
                      ...
                      {etaError && (
                        <div className="text-xs px-3 py-2 rounded-lg bg-red-50 border-red-200 text-red-600">
                          {etaError}
                        </div>
                      )}
                      <div className="flex items-center space-x-2">
                        <input
                          ...
                          className="flex-1 text-xs rounded-lg px-2.5 py-1.5 font-mono focus:outline-none focus:ring-1 focus:ring-blue-500 bg-slate-50 border-token text-slate-850"
                        ...
                        <button
                          ...
                          className="px-3.5 py-1.5 rounded-lg text-[10.5px] font-bold tracking-wider border-none cursor-pointer text-center whitespace-nowrap transition-all shadow-3xs bg-slate-900 hover:bg-slate-850 text-white"
                        >
                          Change ETA
                        </button>
                      </div>
                    ...
                    <p className="text-[10.5px] font-semibold p-2.5 rounded-lg border italic leading-relaxed bg-red-50/50 border-red-100 text-red-650">
              ...
              {(task.ParentTaskID || task.TemplateID || task.FollowUpCount > 0) && (
                <div className="space-y-2">
                  <h4 className="text-[10px] font-bold tracking-wider text-[#64748B]">Lineage references</h4>
                  <div className="border p-3 rounded-lg text-xs space-y-1.5 bg-blue-900/5 border-blue-500/10 text-slate-700">
                    {task.ParentTaskID && (
              ...
              {task.AttachmentLink && (
                <div className="space-y-1.5">
                  <h4 className="text-[10px] font-bold tracking-wider text-[#64748B]">Reference attachment</h4>
                  <a
                    href={task.AttachmentLink}
                    target="_blank"
                    rel="noreferrer"
                    className="flex items-center space-x-2 text-xs border rounded-lg p-3 transition bg-slate-100 hover:bg-slate-200 border-slate-300 text-slate-700"
                  >
                    <LinkIcon size={14} className="text-secondary" />
                    <span className="truncate flex-1 font-mono text-[11px] underline text-blue-600">
                      {task.AttachmentLink}
                    </span>
                  </a>
                </div>
              ...
              {/* Follow-Up Reason */}
              {task.FollowUpReason && (
                <div className="rounded-lg p-3.5 font-sans bg-amber-50 border-amber-200">
                  <div className="flex items-center space-x-1 text-xs font-semibold tracking-wider mb-1 text-amber-800">
                    <CornerRightDown size={14} className="text-amber-600" />
                    <span>Follow-up reason</span>
                  </div>
                  <p className="text-xs italic leading-relaxed text-amber-700">
              ...
              {/* Closing Notes */}
              {task.CloseRemark && (
                <div className="rounded-lg p-3.5 font-sans bg-emerald-50 border-emerald-200">
                  <div className="flex items-center space-x-1 text-xs font-semibold tracking-wider mb-1 text-emerald-800">
                    <CheckCircle size={14} className="text-emerald-600" />
                    <span>Audit close remarks</span>
                  </div>
                  <p className="text-xs italic leading-relaxed text-emerald-700">
            ...
            <div className="space-y-4">
              {taskReports.length === 0 ? (
                ...
                <div className="relative border-l pl-4 ml-2 space-y-4 border-[#E2E8F0]">
                  {taskReports.map((report, rIdx) => {
                    ...
                    return (
                      ...
                      >
                        {/* Timeline Dot */}
                        <span className="absolute -left-[25px] top-4 w-2.5 h-2.5 rounded-full bg-[#2563EB] border-2 shadow-xs border-white" />
                        
                        <div className="flex justify-between items-start">
                          <div className="flex items-center space-x-2">
                            <span className="text-[10px] font-mono font-bold text-secondary">{report.ReportDate}</span>
                            {subtask ? (
                              <span className="text-[9px] border px-2 py-0.5 rounded font-bold tracking-wider bg-purple-50 text-purple-700 border-purple-200">
                                Subtask: {subtask.Title}
                              </span>
                            ) : (
                              <span className="text-[9px] border px-2 py-0.5 rounded font-bold tracking-wider bg-slate-100 text-secondary border-token">
                                Parent task
                              </span>
                            )}
                          </div>
                        ...
                        {report.Blockers && (
                          <div className="mt-2 text-[11px] p-2 rounded border flex items-start space-x-1.5 bg-amber-50 text-amber-800 border-amber-200/50">
                        ...
                        {report.AttachmentLink ? (
                            ...
                            {report.AttachmentLink.split(',').map((url, idx) => {
                              ...
                              return (
                                <a
                                  ...
                                  className="flex items-center space-x-1.5 text-[10px] hover:underline font-bold text-blue-600"
                          ...
                          <div className="mt-2.5 text-[10px] italic text-secondary">
                            No attachment
                          </div>
                        )}

                        <div className="mt-3 pt-2.5 border-t flex items-center justify-between text-[9px] font-mono border-slate-100 text-[#64748B]">
        ...
        <div className="p-4 border-t flex flex-col space-y-2 border-[#E2E8F0] bg-slate-50">
          {/* Close Task form */}
          {showCloseForm && (
            <form onSubmit={handleCloseSubmit} className="rounded-xl p-4.5 space-y-3 mb-2 shadow-sm bg-surface border-[#E2E8F0]">
              <label className="block text-[10px] font-bold tracking-wider text-[#64748B]">
              ...
              <textarea
                ...
                className="w-full text-xs border rounded-lg p-2.5 focus:outline-none focus:ring-1 focus:ring-[#2563EB] bg-surface border-[#E2E8F0] text-slate-800 placeholder-slate-400"
              />

              {/* Attachment link / URL */}
              <div className="space-y-1">
                <label className="block text-[10px] font-bold tracking-wider mb-1 flex items-center space-x-1 text-[#64748B]">
                  <LinkIcon size={12} />
                ...
                <input
                  ...
                  className="w-full text-xs border rounded-lg px-3 py-2 focus:outline-none focus:ring-1 focus:ring-[#2563EB] bg-surface border-[#E2E8F0] text-slate-800 placeholder-slate-400"
                />
              </div>
              ...
              <div className="space-y-1">
                ...
                <div className="border-2 border-dashed rounded-lg p-4 border-[#E2E8F0] bg-slate-50/50">
                  <input
                    type="file"
                    multiple
                    onChange={(e) => {
                  ...
                  >
                    ...
                    <p className="text-[10px] text-secondary text-center mt-1">
                ...
                {uploadedFiles.length > 0 && (
                  <div className="mt-3 space-y-2">
                    <p className="text-[10px] font-bold tracking-wider text-[#64748B]">Uploaded files:</p>
                    {uploadedFiles.map((file, index) => (
                      <div key={index} className="flex items-center justify-between border rounded-lg p-2 bg-slate-50 border-[#E2E8F0]">
                        <div className="flex items-center space-x-2">
                          ...
                          <span className="text-xs truncate max-w-[200px] text-slate-700">{file.name}</span>
                        </div>
                        <button
                          type="button"
              ...
              <div className="flex justify-end space-x-2">
                <button
                  ...
                  className="px-3 py-1.5 rounded-lg text-xs font-bold tracking-wider cursor-pointer text-slate-500 hover:bg-slate-150"
                >
                  Cancel
                </button>
                <button
